...
func _ready() -> void:
	...
	undo_.add_undo_method(MoveMode.move_action_.bind(selection_.duplicate(), get_positions_(selection_)))
...
func _exit_tree():
	...
	undo_.add_do_method(MoveMode.move_action_.bind(selection_.duplicate(), get_positions_(selection_)))
	undo_.commit_action()
