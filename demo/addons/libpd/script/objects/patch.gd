...
func open_patch_in_new_window_() -> void:
	...
	var viewer = load("res://addons/libpd/script/widgets/viewer/viewer.tscn").instantiate()
...
func add_node__(node_name:String):
	var node = preload("res://addons/libpd/script/objects/graph_node.tscn").instantiate()
	node.name = node_name
	node.canvas = self
	node.text = node_name
...
func add_connection_(from_object:int, outlet:int, to_object:int, inlet:int) -> PDCable:
	...
	var cable = preload("res://addons/libpd/script/objects/cable.tscn").instantiate()
