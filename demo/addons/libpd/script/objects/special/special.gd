...
func add_ghost_rs_() -> void:
	...
	var receiver_args = ["obj", str(parent.position.x), str(parent.position.y - 30), "r", '/r/%s/%s' % ["NAMESPACE", parent.index]]
	var receiver_node = preload("res://addons/libpd/script/objects/graph_node.tscn").instantiate()
	...
	var sender_args = ["obj", str(parent.position.x), str(parent.position.y + 30), "s", '/s/%s/%s' % ["NAMESPACE", parent.index]]
	var sender_node = preload("res://addons/libpd/script/objects/graph_node.tscn").instantiate()
	...
	var receiver_cable = preload("res://addons/libpd/script/objects/cable.tscn").instantiate()
	...
	var sender_cable = preload("res://addons/libpd/script/objects/cable.tscn").instantiate()
...
func animate_(sustain:float = 0.5, attack:float = 0.1, decay:float = 0.1) -> void:
	...
	tween_ = create_tween()
	tween_.tween_property(self, "self_modulate", self_modulate * 1.1, attack)
	tween_.tween_property(self, "self_modulate", Color.WHITE, decay).set_delay(sustain)
