...
func _ready() -> void:
	#area_entered.connect(_mouse_entered)
	#area_exited.connect(_mouse_exited)
...
func get_best_slot(slot:GraphControlSlot) -> GraphControlSlot:
	var test_slots
	if slot.is_output:
...
func _select():
	...
	if %Specialized.get_child_count() > 0:
		if %Specialized.get_child(0).has_method("_select"):
			%Specialized.get_child(0)._select()
...
func create_obj_(message:String) -> Array:
	...
	if message_type == 'obj':
		...
		node_model = node_db.get_node_model(arg_parser.next())
	elif message_type == 'coords':
		node_model = node_db.db.get(message_type)
