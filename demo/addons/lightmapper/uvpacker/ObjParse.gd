...
static func load_obj(path: String) -> Mesh:
	var file = FileAccess.open(path, FileAccess.READ)
	var data = file.get_as_text()
	return load_obj_from_buffer(data, {})

# Create mesh from obj and mtl paths
static func load_obj_mtl(obj_path: String, mtl_path: String = "") -> Mesh:
...
static func _get_mtl_tex(mtl_path: String) -> Dictionary:
	var file_paths: Array = _get_mtl_tex_paths(mtl_path)
...
static func _get_mtl_tex_paths(mtl_path: String) -> Array:
...
static func _get_image(mtl_filepath: String, tex_filename: String) -> Image:
	...
	var _err = img.load(tex_filepath)
	return img

static func _create_texture(data) -> ImageTexture:
	var img: Image = Image.new()
	var _err = img.load_png_from_buffer(data)
				...
				if (parts.size() == 4):
					...
					for i in (points.size()):
						if (i != 0):
							...
							var point1: Array = points[i]
							var point2: Array = points[i-1]
				...
				if (face["vt"].size() > 0):
					for k in [0,2,1]:
						...
						if (f > -1):
							var uv = uvs[f]
							fan_vt.append(uv)
				st.add_triangle_fan(fan_v, fan_vt, PackedColorArray(), fan_vt, fan_vn, [])
		mesh = st.commit(mesh)
...
static func convert_to_obj(mesh: Mesh, object_name: String):
	...
	for s in range(mesh.get_surface_count()):
		...
		var uvs = surface[ArrayMesh.ARRAY_TEX_UV2]
		var normals = surface[ArrayMesh.ARRAY_NORMAL]
		
		for v in vertices:
			output += str("v ", v.x, " ",v.y, " ", v.z, "\n")
		
		var has_uv = false
		if uvs != null:
			for uv in uvs:
				output += str("vt ", uv.x, " ", 1.0 - uv.y , "\n")
			has_uv = true
		
		var has_n = false
		if normals != null:
			for n in normals:
				output += str("vn ", n.x, " " , n.y, " " , n.z, "\n")
			has_n = true
		
		#var time2 = OS.get_ticks_msec()
		...
		if has_uv and has_n:
			for i in range(0, indices.size(), 3):
				var id1 = index_base + indices[i]
				var id2 = index_base + indices[i + 2]
				var id3 = index_base + indices[i + 1]
				output += str("f ", id1, "/", id1, "/", id1, " ", id2, "/", id2, "/", id2, " ", id3, "/", id3, "/", id3, "\n")
		elif has_uv and !has_n:
			for i in range(0, indices.size(), 3):
				var id1 = index_base + indices[i]
				var id2 = index_base + indices[i + 2]
				var id3 = index_base + indices[i + 1]
				output += str("f ", id1, "/", id1, " ", id2, "/", id2, " ", id3, "/", id3, "\n")
		elif !has_uv and has_n:
			for i in range(0, indices.size(), 3):
				var id1 = index_base + indices[i]
				var id2 = index_base + indices[i + 2]
			...
			for i in range(0, indices.size(), 3):
				...
				var id2 = index_base + indices[i + 2]
				var id3 = index_base + indices[i + 1]
				output += str("f ", id1, " ", id2, " ", id3, "\n")
		
		index_base += surface[ArrayMesh.ARRAY_VERTEX].size()
		#print(str("Mesh saved: ", OS.get_ticks_msec() - time, " ms"))
		#print(str("part2: ", OS.get_ticks_msec() - time2, " ms"))
